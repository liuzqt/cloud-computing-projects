 ...
 * driver matches.
 */
class DriverMatchTask_bonus implements StreamTask, InitableTask, WindowableTask {
    ...
    private KeyValueStore<String, String> history;
    ...
    public void init(Config config, TaskContext context) throws Exception {
        ...
        history = (KeyValueStore<String, String>) context.getStore("history");
...
//						continue;
                ...
                if (currentDriver != null) {
                    ...
                    String temp = history.get(blockid);
                    KeyValueIterator<String, String> countIter = driver.range(blockid + ":", blockid + ";");
                    int n = 0;
                    while (countIter.hasNext()) {
                        countIter.next();
                        n++;
                    }
                    history.put(blockid, String.valueOf(n));
                    if (temp == null) {
                    } else {
                        double avg = (Double.parseDouble(temp) + n * 1.0) / 2;
                        if (avg >= 4.5)
                            ;
                        else {
                            double sf = 2 * (4.5 - avg) / (1.25);
                            spf = sf + 1;
                        }
                    }

                    driver.delete(blockid + ":" + currentDriver);
                    HashMap<String, String> output = new HashMap<>();
                    output.put("clientId", clientid);
                    output.put("driverId", currentDriver);
                    output.put("priceFactor", String.valueOf(spf));
